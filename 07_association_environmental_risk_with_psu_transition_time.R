...
data_fea =  data_fea[which(data_fea$analysis_sample_y0_6_5_survive==1),]

#------------------------ logistic regression with psu as outcome
for (type_psu in c("NSU2SSU",  "NSU2PSU", "SSU2PSU")){
  ...
  for (num_fea in num_environ){    
    ...
    if (type_psu != "SSU2PSU"){
      data_combine = data.frame(IID = data_fea$participant_id, environ = data_fea[, num_fea], survived_time = data_fea[, type_psu], 
    ...
    data_combine = na.omit(data_combine[, c('IID', 'environ', 'survived_time', 'flag.transition', 'age', 'sex', 'Tanner_stage', 'INR', 'race', 'site', 'familyID')])
    ...
    if (dim(data_combine)[1]>500){
      ...
      hr <- coxme(Surv(survived_time, flag.transition) ~ environ + age + sex + Tanner_stage + INR + race + (1|familyID) + (1|site), data = data_combine)
      
      coeff = summary(hr)$coefficients
      
      # extract each term, and interaction term
      out1 = c(round(coeff["environ", c("exp(coef)", "se(coef)", "z", "coef")], 3), sprintf("%.2e", coeff["environ", "p"]))
  ...
  write.csv(out, file = paste0(proj_dir, 'experiment/Table.Step.5.psu.survival.time.environment.association.', type_psu, '.03.25.2026.csv',sep=""), row.names = TRUE)
}
