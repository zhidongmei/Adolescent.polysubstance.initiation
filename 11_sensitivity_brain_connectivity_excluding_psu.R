...
# ========================================================
for (time_point in c("ses-04A", "ses-02A", "ses-00A")){
  
  time_point0 = gsub("-", ".", time_point)
  
  for (img_file in c("mr_y_rsfmri__corr__gpnet__aseg", "mr_y_rsfmri__corr__gpnet")){#"mr_y_rsfmri__corr__gpnet", 
    ...
    for (type_psu in c("psu_ssu_0_6_5")){ #"psu_0_6_5", "ssu_0_6_5", "psu_ssu_0_6_5", "psu_sum_0_6_5"
      ...
      type_psu_base = c("ses-00A" = "psu_0_0", "ses-02A" = "psu_0_2", "ses-04A" = "psu_0_4")[time_point]
      ...
      for (num_fea in num_image){    
        ...
        data_combine = data.frame(IID = data_fea$participant_id, psu_out = data_fea[, type_psu], image_fea = data_fea[, num_fea], 
                                  psu_out_base = data_fea[, type_psu_base],
        ...
        data_combine = na.omit(data_combine[, c('IID', 'psu_out', 'psu_out_base', 'image_fea', 'motion', 'age', 'sex', 'INR', 'race', 'site', 
                                                'scanner', 'familyID', 'Tanner_stage')])
        data_combine <- data_combine %>% 
          filter(psu_out_base != 1)
        ...
        data_combine <- data_combine[!duplicated(data_combine$familyID), ]
        
        if (dim(data_combine)[1]>500){
          
          model <- glmmTMB(psu_out ~ image_fea + age + sex + motion + Tanner_stage + INR + race + (1 | scanner) + (1 | site), data = data_combine, family = binomial(link = "logit"))  # delete scanner
      ...
      write.csv(out, file = paste0(proj_dir, 'experiment/Table.8.2.psu.imaging.association.', type_psu, '.', time_point, '.', img_file, '.meanFD_0.2mm.ses.exclude.psu.base.03.17.2026.csv',sep=""), row.names = TRUE)
    }
  }
}
